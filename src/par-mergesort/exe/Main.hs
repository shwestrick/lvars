...
main =
  do
    ...
    let rounds = (read $ args !! 1) :: Int
    let warmupRounds = (read $ args !! 2) :: Int
    putStrLn $ "size " ++ show size
    putStrLn ""
    let input = SV.generate size (hash . fromIntegral)
    putStrLn $ input `deepseq` (show $ input SV.! (size-1))
    if warmupRounds == 0
      then return () -- just exits the if, we will still do stuff below.
      else do
        putStrLn $ "warming up"
        warmup 0 warmupRounds input
    putStrLn $ "running experiments"
    putStrLn ""
    runMany 0 rounds input
...
runMany i n input =
  if i == n then return () else do
    putStrLn $ "==== run " ++ show i ++ " ===="
    t0 <- getCurrentTime
    let result = sortPV 10000 10000 CSort CMerge input
    -- putStrLn $ (show $ result SV.! 0) ++ " " ++ (show $ result SV.! 42)
    t1 <- result `deepseq` getCurrentTime
    putStrLn $ "wall " ++ (show $ diffInMilliseconds t1 t0)
    putStrLn ""
    runMany (i+1) n input
...
warmup i n input =
  if i == n then return () else do
    ...
    -- putStrLn $ (show $ result SV.! 0) ++ " " ++ (show $ result SV.! 42)
    t1 <- result `deepseq` getCurrentTime
    putStrLn $ (show $ diffInMilliseconds t1 t0)
    warmup (i+1) n input

diffInMilliseconds :: UTCTime -> UTCTime -> Int
diffInMilliseconds t1 t0 =
  round ((realToFrac (diffUTCTime t1 t0) :: Double) * 1000.0)
